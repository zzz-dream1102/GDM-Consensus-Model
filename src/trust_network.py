...
class GraphAttentionLayer(nn.Module):
    # ... (保持原有的 __init__ 和 forward 不变，这部分没问题) ...
...
class TrustCompletionModel(nn.Module):
    ...
    def compute_loss(self, pred_matrix, true_matrix, obs_mask):
        """
        [STRICT FIX] 严格对应公式 (9): 误差平方和 (SSE)。
        原始代码错误地实现了 MSE (除以了元素个数)，这会导致 loss 值过小，
        使得正则化项 lambda_reg 权重过大，模型学不到东西。
        """
        # Part 1: Masked Sum of Squared Errors (SSE)
        # 公式: sum_{i,k} M_{ik} (hat_t - t)^2
        diff = pred_matrix - true_matrix
        squared_diff = diff ** 2
        
        # 修正：移除分母 / (torch.sum(obs_mask) + 1e-9)
